  ...
  <div class="my-[42px] bg-white p-6">
    ...
    <div class="py-10 flex gap-10">
      <div class="w-1/2">
        ...
        <image-upload
          class="h-full"
          @updateImageUrl="handleUpdateImageUrl"
          v-if="!imageUrl"
        />
        <img :src="imageUrl" alt="" class="w-full h-1/2 object-cover" v-else />
        <p class="mt-5" v-if="imageUrl">Upload new Image</p>
        <image-upload
          class="h-[200px]"
          @updateImageUrl="handleUpdateImageUrl"
          v-if="imageUrl"
...
const fetchABlogPost = async (id) => {
  const data = await BlogService.fetchABlogPost(route.params.id);
  title.value = data.title;
  body.value = data.body;
  author.value = data.author;
  selectedCategories.value = data.categories;
  imageUrl.value = data.mainImageUrl;
  console.log(data);
};
...
const handleCreateBlog = async () => {
  ...
  try {
    ...
    const data = await BlogService.updateBlogPost(route.params.id, payload);
