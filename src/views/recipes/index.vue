...
const fetchProducts = async (params) => {
  const data = await RecipeService.fetchRecipes(params);
...
const fetchFeaturedProducts = async (params) => {
  const data = await RecipeService.fetchFeaturedRecipes(params);
  total.value = data.total;
  console.log(data);
  blogArray.value = data;
};
...
const handleAddToFeaturedProducts = async () => {
  const payload = {
    recipeId: postId.value._id,
    category: postId.value.category,
  };

  try {
    console.log(postId.value);
    isDeleting.value = true;
    const data = await RecipeService.addToFeaturedRecipes(payload);
    console.log(data);
...
const handleRemoveFromFeaturedProducts = async () => {
  ...
  try {
    console.log(postId.value);
    isDeleting.value = true;
    const data = await RecipeService.removeFromFeaturedProducts(payload);
    console.log(data);
    toast.success(data.responseMessage);
    isDeleting.value = false;
    showFeaturedModal.value = false;
